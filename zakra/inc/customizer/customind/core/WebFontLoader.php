...
class WebFontLoader {
	...
	public function register_monthly_schedule( $schedules ) {
		if ( ! isset( $schedules['monthly'] ) ) {
			$schedules['monthly'] = array(
				...
				'display'  => __( 'Once Monthly', 'customind' ),
			);
		}
		return $schedules;
	}
	...
	public function get_base_path() {
		if ( ! $this->base_path ) {
			$this->base_path = $this->apply_filters( 'font-loader:base-path', $this->get_filesystem()->wp_content_dir() );
		}
	...
	public function get_base_url() {
		if ( ! $this->base_url ) {
			$this->base_url = $this->apply_filters( 'font-loader:base-url', content_url() );
		}
	...
	public function get_subfolder_name() {
		if ( ! $this->subfolder_name ) {
			$this->subfolder_name = $this->apply_filters( 'font-loader:sub-folder-name', 'fonts' );
		}
	...
	public function schedule_cleanup() {
		if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
			if ( ! wp_next_scheduled( 'customind:font-loader:cleanup' ) && ! wp_installing() ) {
				wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'customind:font-loader:cleanup' );
			}
		}
	}
